...
| **Ưu tiên #2** | **WSL Native (Linux)** | **Stdio Transport** (`--transport stdio`) | Dành cho Developers dùng Linux/WSL shell.<br/>• IDE (VS Code Remote-WSL / Cursor) tự spawn `python3`.<br/>• Hiệu năng xử lý cao, ổn định tuyệt đối. |
| **Nâng cao #3** | **Docker Container** | **SSE Transport** (`http://localhost:8000/sse`) | Dành cho DevOps hoặc triển khai Server tập trung:<br/>• Container `sgu_mcp_academic_server` chạy nền.<br/>• Map port `8000:8000`, phục vụ qua HTTP SSE. |
...
### Cấu hình mặc định (Stdio - Khuyên dùng):
Chỉ cần nhấp đúp vào `setup.bat` (trên Windows) hoặc chạy:
```bash
python setup.py
```
Script sẽ tự động nhận diện đường dẫn Python hiện tại và ghi cấu hình Stdio chuẩn vào:
...
### Cấu hình tùy chọn (SSE cho Docker / Remote):
Nếu người dùng muốn kết nối qua Docker SSE ở port 8000:
```bash
python setup.py --transport sse --port 8000
```
