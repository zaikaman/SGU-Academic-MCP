...
> 2. **Docker Container (Stdio Bridge & SSE):** Tự động phát hiện container `sgu_mcp_academic_server` (hỗ trợ cả Docker Desktop Native và Docker qua WSL 2), tạo profile `.cursor/mcp.docker.json` & `.vscode/mcp.docker.json`. Khi muốn toàn bộ IDE ưu tiên chạy qua Docker, chỉ cần thêm cờ: `python setup.py --docker`.
> 3. **WSL Native:** Tự động phát hiện WSL (`python3`, chuyển đổi đường dẫn sang `/mnt/...`) và tạo sẵn profile `.cursor/mcp.wsl.json` & `.vscode/mcp.wsl.json`.
>
