...
class SguApiClient:
    ...
    def __init__(self, base_url: str | None = None):
        ...
        self.access_token: str | None = settings.bearer_token
        self.current_student_id: str | None = settings.student_id

    def _get_headers(self, endpoint: str, token: str | None = None) -> dict[str, str]:
    ...
    async def login(
        ...
        async with httpx.AsyncClient(timeout=15.0, verify=False) as client:  # nosec B501
    ...
    async def _post_api(
        ...
        if not self.access_token:
            ...
            if settings.student_id and settings.password:
                ...
                raise PermissionError(
                    "Chưa có Bearer Token SGU. Vui lòng đăng nhập qua tool login_sgu() hoặc cung cấp SGU_BEARER_TOKEN trong .env"
                )
        ...
        async with httpx.AsyncClient(timeout=20.0, verify=False) as client:  # nosec B501
    ...
    async def get_weekly_schedule(self, semester_id: str | None = None) -> dict[str, Any]:
    ...
    async def get_exam_schedule(self, semester_id: str | None = None) -> dict[str, Any]:
    ...
    async def get_grades(self, semester_id: str | None = None) -> dict[str, Any]:
