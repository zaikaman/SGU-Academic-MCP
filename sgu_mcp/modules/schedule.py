...
async def tool_get_registered_courses() -> dict[str, Any]:
    ...
    return {
        "tong_so_mon": len(courses),
        "danh_sach_mon_dang_ky": courses
    }


async def tool_get_weekly_schedule(semester_id: Optional[str] = None) -> dict[str, Any]:
    """
    ...
    return {
        "thoi_khoa_bieu_tuan": list(day_schedule.values())
    }
...
async def tool_check_schedule_conflict(target_thu: int, target_tiet_bd: int, target_so_tiet: int) -> dict[str, Any]:
    """
            ...
            if s.get("thu_so") == target_thu:
                ...
                if overlap:
                    conflicts.append({
                        ...
                        "ma_mon": c.get("ma_mon"),
                        "tiet_bi_trung": sorted(list(overlap)),
